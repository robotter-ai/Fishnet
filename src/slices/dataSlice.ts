...
const dataApiSlice = fishnetApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    updateDatasetAvailability: builder.mutation<any, IUpdateDatasetAvailability>({
      query: (req) => ({
        method: 'PUT',
    ...
    getViews: builder.query<any, string>({
      query: (datasetID) => `/datasets/${datasetID}/views`,
    ...
    downloadDatasetCsv: builder.query<any, string>({
      ...
      transformResponse: (response) => new Blob([response as string], { type: 'text/csv' }),
    }),
    getDatasetTimeseries: builder.query<any, string>({
      query: (datasetID) => `/datasets/${datasetID}/timeseries`,
    }),
  }),
});
